...
class RowWriter:
    ...
    def asDollarStr(self, nVal):
        return "${:.2f}".format(nVal)
    ...
    def getRebateInfo(self):
        desc = ""
        amnt = 0
        if self.row['rebate_deposit']:
            desc = ", minus deposit"
            amnt = self.row['deposit']
        return desc, amnt
    ...
    def drawItemized(self):
        ...
        rebate_desc, rebate_amnt = self.getRebateInfo()
        if not math.isnan(self.row['flat_rate']):
            desc = "Flat charge for {}{}.".format(self.getDescription(), rebate_desc)
            date = self.getBestAppointmentDate()
            amnt = self.asDollarStr(self.row['flat_rate'] - rebate_amnt)
            itemizedList.append([desc, date, "", amnt])
        elif not math.isnan(self.row['hours']) and math.isnan(self.row['flat_rate']):
            desc = "Hourly charge for {} @{} hours{}.".format(self.getDescription(), self.getData('hours'), rebate_desc)
            date = self.getBestAppointmentDate()
            rate = "$180.00"
            amnt = self.asDollarStr(self.row['hours'] * self.row['hourly_rate'] - rebate_amnt)
